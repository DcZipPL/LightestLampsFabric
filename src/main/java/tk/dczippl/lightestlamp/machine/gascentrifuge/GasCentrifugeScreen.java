...
public class GasCentrifugeScreen extends HandledScreen<GasCentrifugeScreenHandler>
{
    ...
    protected void drawGuiContainerForegroundLayer(MatrixStack matrixStack, int x, int y) {
        ...
        int tmp = 75;
        this.textRenderer.draw(matrixStack, this.title.asOrderedText(), (float)(this.xSize / 2 - tmp / 2), 6.0F, 4210752);
        this.textRenderer.draw(matrixStack, this.playerInventory.getDisplayName().func_241878_f(), 8.0F, (float)(this.ySize - 96 + 2), 4210752);
    ...
    protected void drawGuiContainerBackgroundLayer(MatrixStack matrixStack, float partialTicks, int mouseX, int mouseY) {
        ...
        if (((GasCentrifugeScreenHandler)this.container).func_217061_l()) {
            int k = ((GasCentrifugeScreenHandler)this.container).getBurnLeftScaled()*2;
            if (k >= 300)
                k = 299;
            //Z Y T-Z T-Y W H
            this.blit(matrixStack,i + 151, j + 65 + 12 - k, 203, 118 - k, 9, k + 1);
        }
        int m = ((GasCentrifugeScreenHandler)this.container).getLiquidScaled();
        //Z Y T-Z T-Y W H
        ...
        int l = ((GasCentrifugeScreenHandler)this.container).getCookProgressionScaled();
