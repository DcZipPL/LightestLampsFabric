...
public class GasCentrifugeRecipeCategory implements IRecipeCategory<GasCentrifugeRecipeJEI>
{
    ...
    public GasCentrifugeRecipeCategory(IGuiHelper helper)
    {
        ...
        icon = helper.createDrawableIngredient(new ItemStack(ModBlocks.GAS_EXTRACTOR.get()));
    }
    ...
    @Override
    public void draw(GasCentrifugeRecipeJEI recipe, MatrixStack matrixStack, double mouseX, double mouseY) {
        IRecipeCategory.super.draw(recipe, matrixStack, mouseX, mouseY);
        recipe.drawInfo(Minecraft.getInstance(), matrixStack, 0, 40, mouseX, mouseY);
    }
}